...
The default configuration requires a directory called 'files' in the public folder. You can change this by publishing the config file.
    ...
    php artisan vendor:publish --provider='Perederko\Laravel\Ext\Admin\ElFinder\ElFinderServiceProvider' --tag=config

In your config/admin-elfinder.php, you can change the default folder, the access callback or define your own roots.
...
You can override the default views by copying the resources/views folder. You can also do that with the vendor:publish command:
    ...
    php artisan vendor:publish --provider='Perederko\Laravel\Ext\Admin\ElFinder\ElFinderServiceProvider'  --tag=views
...
You can use tinyMCE integration with the following route, which by default is `admin/elfinder/tinymce4`:

```php
route('admin-elfinder.tinymce4');
```
...
function elFinderBrowser (field_name, url, type, win) {
  tinymce.activeEditor.windowManager.open({
    file: '<?= route('admin-elfinder.tinymce4') ?>',// use an absolute path!
    title: 'elFinder 2.0',
    width: 900,
    height: 450,
    resizable: 'yes'
...
You can add tinyMCE integration with the following route (default: `admin/elfinder/tinymce`):

    route('admin-elfinder.tinymce');
...
```javascript
function elFinderBrowser (field_name, url, type, win) {
  var elfinder_url = 'admin/elfinder/tinymce';    // use an absolute path!
  tinyMCE.activeEditor.windowManager.open({
    file: elfinder_url,
    title: 'elFinder 2.0',
    width: 900,
...
You can add the popup with the following action:
    ...
    'Perederko\Laravel\Ext\Admin\ElFinder\ElFinderController@popup'
...
```html
<script type="text/javascript" src="/packages/perederko/laravel-admin-elfinder-ext/js/standalonepopup.min.js"></script>
```
